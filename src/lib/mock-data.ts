...
export async function checkUserExistsInMock(details: { email: string; username: string; phone: string; }): Promise<boolean> {
    if (!db) return false;
    const usersRef = collection(db, "users");
    const emailQuery = query(usersRef, where("contact", "==", details.email), limit(1));
    const usernameQuery = query(usersRef, where("username", "==", details.username), limit(1));
    const phoneQuery = query(usersRef, where("phone", "==", details.phone), limit(1));
    ...
    return !emailSnap.empty || !usernameSnap.empty || !phoneSnap.empty;
}

export async function registerCustomerInMock(data: CustomerRegistrationFormValues): Promise<UserProfile> {
    ...
    const newUser: Omit<UserProfile, 'id'> = {
        ...
        approvalStatus: 'Pending', photoURL: `https://placehold.co/100x100.png?text=${data.name.charAt(0)}`,
...
export async function registerTrainerInMock(data: TrainerRegistrationFormValues) {
    if (!db) throw new Error("Database not configured");
    const userRef = doc(collection(db, 'users'));
    const newUser: Omit<UserProfile, 'id'> = {
        ...
        approvalStatus: 'Pending', photoURL: `https://placehold.co/100x100.png?text=${data.name.charAt(0)}`,
        myReferralCode: `${data.name.split(' ')[0].toUpperCase()}${generateId().slice(-4)}`,
        vehicleInfo: data.trainerVehicleType, specialization: data.specialization,
...
export async function completeCustomerProfileInMock(userId: string, data: FullCustomerDetailsValues) {
    if (!db) throw new Error("Database not configured");
    const userRef = doc(db, 'users', userId);
    const getLessonsForPlan = (plan: string): number => ({ Premium: 20, Gold: 15, Basic: 10 }[plan] || 0);
    
    const updateData = {
        subscriptionPlan: data.subscriptionPlan, vehicleInfo: data.vehiclePreference,
        trainerPreference: data.trainerPreference, flatHouseNumber: data.flatHouseNumber,
        street: data.street, district: data.district, state: data.state, pincode: data.pincode,
        location: data.district, dlStatus: data.dlStatus, dlNumber: data.dlNumber || '',
        dlTypeHeld: data.dlTypeHeld || '', photoIdType: data.photoIdType,
        photoIdNumber: data.photoIdNumber, photoIdUrl: data.photoIdUrl,
        subscriptionStartDate: format(data.subscriptionStartDate, 'MMM dd, yyyy'),
        totalLessons: getLessonsForPlan(data.subscriptionPlan), completedLessons: 0,
    ...
    const newRequest: Omit<LessonRequest, 'id'> = {
        ...
        vehicleType: data.vehiclePreference as VehicleType, status: 'Pending',
        requestTimestamp: new Date().toISOString(),
    };
    await addDoc(collection(db, 'lessonRequests'), newRequest);
}
